  ...
  <div v-loading="loading">
    <div class="addinfo">
      ...
      >
        ...
        <el-form-item prop="zoneCode" class="formitem">
          <span slot="label"><label class="required-icon">*</label> 库区</span>
          <div class="bombor">
            <el-select
              v-model="addData.zoneCode"
              :disabled="type == 1 ? true : false"
              size="small"
              data-checked
              :data-checkedValue="addData.zoneCode"
              data-checkedTitle="请选择库区"
              class="select"
            >
              <el-option
        ...
        <el-form-item prop="locationName" class="formitem left">
          <span slot="label"
            ><label class="required-icon">*</label> 库区名称</span
          >
          <div class="bombor">
            <el-input
              v-model="addData.locationName"
              placeholder="请输入"
              data-checked
              :data-checkedValue="addData.locationName"
        ...
        <el-form-item prop="lockStatus" class="formitem">
          ...
          >
          <el-radio-group v-model="addData.lockStatus">
  ...
  methods: {
    ...
    selectWarehouse(val) {
      if (val) {
        this.addData.zoneCode = "";
        this.storeAreaList = [];
        this.getStoreAreaList(val);
      }
    },
    // 库区列表
    getStoreAreaList(warehouseCode) {
      storeAreaList({
        isPage: false,
    ...
    getDetail() {
      storeLocationDetail({
        ...
        if (res.code == 0) {
          ...
          this.getStoreAreaList(res.data.warehouseCode);
    ...
    saveAdd() {
      ...
      if (fromChecked()) {
        ...
        let saveFun =
          type == 0
            ? storeLocationAdd(this.addData)
            : storeLocationEdit(this.addData);
        saveFun.then(res => {
          this.loading = false;
          if (res.code == 0) {
            this.$message.success(type == 0 ? "新增成功" : "编辑成功");
            this.$router.push({ path: "/main/basicData/storeLocation/list" });
          } else {
            this.$message.error(res.msg);
          }
        });
